...
async function editPdf(data) {
  ...
  const { nom, prenom, datenaissance, lieunaissance, adresse, codepostal, ville } = JSON.parse(data);
  ...
  drawText(new Date().getFrenchFormat(), 110, 155);
  ...
  for (i = 0; i < 9; i++) {
    drawText("x", crossCoords[i][0], crossCoords[i][1]);
  }
